...
if (minipdf_lib === 'pdf.js') {
	//minipdf_lib = require('../core/minipdf_js.js');
	console.log('pdf');
	...
	if (doc.acroForm) {
		...
		while (to_visit.length > 0) {
			...
			if (n.map && n.map.Kids && n.map.FT &&(n.map.FT.name === 'Btn')) {
				...
				if(n.map.Opt){
					n._pdfform_spec.options= n.map.Opt;
				} else {
					n._pdfform_spec.options= ['YES','NO','UNKNOWN','NA'];
				}
				callback(n);
	...
	visit_acroform_fields(doc, function(n) {
		...
		if (n._pdfform_spec) {
			...
			if (type === 'radio') {
				...
				var parent = doc.fetch(kid.map.Parent);
				kid.map.AS = new minipdf_lib.Name(value);
				parent.map.V = new minipdf_lib.Name(value);
				//resetting default for pre selected kid acroform
				if(parent.map.Kids){
					const kids = parent.map.Kids;
					kids.map((selected)=>{
						const selectedKid = doc.fetch(selected);
						if(selectedKid.map.AS.name!=value){
							if(selectedKid.map.AS.name!='Off'){
								selectedKid.map.AS = new minipdf_lib.Name('Off');
								var selected_kid_entry = objects.update(selected,selectedKid);
								objects.write_object(out,selected_kid_entry);
							}
						}
					})
				}
				//changing acroform for parent and kind on selection
				var parent_entry = objects.update(parent._pdfform_ref,parent);
				var kid_entry = objects.update(kid_ref, kid);
				objects.write_object(out, parent_entry);
...
pdfform.transform = function(buf, fields) {return pdfform('pdf.js').transform(buf, fields);};

if (typeof module != 'undefined') {
	module.exports.func = pdfform(minipdf_js).transform;
}
